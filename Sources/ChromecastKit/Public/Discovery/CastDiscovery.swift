...
public actor CastDiscovery {
  ...
  public func device(
    ...
    devices().first { device in
      if caseInsensitive {
        return device.friendlyName.compare(
          name, options: [.caseInsensitive, .diacriticInsensitive]) == .orderedSame
      }
  ...
  public func waitForDevice(
    ...
    return try await waitForDeviceFromEvents(
      ...
      guard case .deviceUpserted(let device, _) = event, device.id == id else {
        return nil
      }
      return device
    }
  ...
  public func waitForDevice(
    ...
    return try await waitForDeviceFromEvents(
      ...
      guard case .deviceUpserted(let device, _) = event else {
        return nil
      }
      let matches: Bool
      if caseInsensitive {
        matches =
          device.friendlyName.compare(name, options: [.caseInsensitive, .diacriticInsensitive])
          == .orderedSame
      } else {
  ...
  func upsertDiscoveredDevice(_ device: CastDeviceDescriptor) {
    ...
    if let existingID = existingDeviceID(matching: device) {
      ...
      let merged = mergeDiscoveredDevice(
        existing: existing, incoming: device, preservingID: existingID)
  ...
  private func handleBrowserEvent(_ event: CastDiscoveryBrowserEvent) async {
    switch event {
    case .deviceUpserted(let device):
      upsertDiscoveredDevice(device)
