...
enum CastSSDPDiscoveryParser {
    ...
    static func makeDescriptor(
        ...
        let fallbackID: CastDeviceID =
            uuid.map { CastDeviceID($0.uuidString.lowercased()) }
    ...
    private static func normalizeDeviceID(fromUDN udn: String) -> CastDeviceID? {
        ...
        let normalized =
            trimmed.lowercased().hasPrefix("uuid:") ? String(trimmed.dropFirst(5)) : trimmed
...
extension CastSSDPDiscoveryParser {
    ...
    fileprivate final class XMLDeviceDescriptionDelegate: NSObject, XMLParserDelegate {
