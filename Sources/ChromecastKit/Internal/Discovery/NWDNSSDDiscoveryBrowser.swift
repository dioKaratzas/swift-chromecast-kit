...
actor NWDNSSDDiscoveryBrowser: CastDiscoveryBrowser {
  ...
  private var discoveredDeviceIDsByService = [
    CastBonjourDiscoveryParser.ServiceIdentity: CastDeviceID
  ]()
  ...
  private func handleAddedOrChangedResult(_ result: NWBrowser.Result) {
    guard let service = CastBonjourDiscoveryParser.serviceIdentity(from: result.endpoint),
      service.type == CastBonjourDiscoveryParser.serviceType
    else {
      return
    }
    ...
    resolutionTasks[service] = Task {
      do {
        ...
        guard
          CastBonjourDiscoveryParser.shouldInclude(
            descriptor,
  ...
  private static func resolveEndpoint(for endpoint: NWEndpoint) async throws
    -> CastBonjourDiscoveryParser.ResolvedEndpoint
  {
    switch endpoint {
    ...
    return try await withThrowingTaskGroup(of: CastBonjourDiscoveryParser.ResolvedEndpoint.self) {
      group in
      group.addTask {
  ...
  private static func resolveEndpointByConnecting(to endpoint: NWEndpoint) async throws
    -> CastBonjourDiscoveryParser.ResolvedEndpoint
  {
  ...
  private static func endpointFromNWEndpoint(_ endpoint: NWEndpoint) throws
    -> CastBonjourDiscoveryParser.ResolvedEndpoint
  {
...
extension NWDNSSDDiscoveryBrowser {
  ...
  fileprivate final class ConnectionResolutionBox: @unchecked Sendable {
    private let lock = NSLock()
    private var continuation:
      CheckedContinuation<CastBonjourDiscoveryParser.ResolvedEndpoint, any Error>?
    private var isCompleted = false

    init(connection _: NWConnection) {}

    func install(
      _ continuation: CheckedContinuation<CastBonjourDiscoveryParser.ResolvedEndpoint, any Error>
    ) {
      lock.lock()
