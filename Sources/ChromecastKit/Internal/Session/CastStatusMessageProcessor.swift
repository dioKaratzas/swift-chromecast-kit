  ...
  func apply(_ message: CastInboundMessage) async throws -> Bool {
    ...
    case (.receiver, CastReceiverMessageType.receiverStatus.rawValue):
      let response = try CastMessageJSONCodec.decodePayload(
        CastWire.Receiver.StatusResponse.self, from: message)
    ...
    case (.media, CastMediaMessageType.mediaStatus.rawValue):
      let response = try CastMessageJSONCodec.decodePayload(
        CastWire.Media.StatusResponse.self, from: message)
    ...
    case (.multizone, CastMultizoneMessageType.multizoneStatus.rawValue):
      let response = try CastMessageJSONCodec.decodePayload(
        CastWire.Multizone.StatusResponse.self, from: message)
...
private func mapCastingGroup(_ group: CastWire.Multizone.CastingGroupsResponse.Group)
  -> CastCastingGroup?
{
...
private func mapMediaMetadata(_ metadata: CastWire.Media.Metadata) -> CastMediaMetadata? {
  let images =
    metadata.images?.compactMap { image -> CastImage? in
  ...
  case 1:
    return .movie(
      .init(
        ...
        releaseDate: metadata.releaseDate.flatMap(CastISO8601Coding.parse(_:)),
        images: images
      )
    )
  case 2:
