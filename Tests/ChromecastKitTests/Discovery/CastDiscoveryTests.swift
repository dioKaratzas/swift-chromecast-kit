...
struct CastDiscoveryTests {
  ...
  func deviceUpsertAndRemoval() async {
    ...
    guard case .deviceUpserted(let first, isNew: let firstIsNew)? = await events.next() else {
    ...
    guard case .deviceUpserted(let second, isNew: let secondIsNew)? = await events.next() else {
  ...
  func browserRuntimeErrorStopsAndCanRestart() async throws {
    ...
    guard case .error(let error)? = errorEvent else {
  ...
  func deduplicatesDiscoveredDevicesAcrossBackends() async {
    ...
    guard case .deviceUpserted(let device, let isNew)? = await events.next() else {
